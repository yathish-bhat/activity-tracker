  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-4 mb-7" style={{ animation: 'fadeUp 0.6s 0.1s ease both' }}>
        {Object.entries(ACTIVITY_TYPES).map(([key, info]) => {
          ...
          return (
            ...
            >
              <div className="absolute top-0 left-0 right-0 h-[2px] rounded-t-[20px]" style={{ background: info.color }} />
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-[10px]" style={{ color: 'var(--muted)' }}>{info.desc}</div>
                </div>
              </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 gap-3" style={{ animation: 'fadeUp 0.6s 0.3s ease both' }}>
          {recent.map(a => {
            const info = getTypeInfo(a.type);
            return (
              <div key={a.id}
                className="flex items-center gap-4 rounded-[20px] p-5 transition-all duration-200 hover:-translate-y-0.5"
                style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.12)'}
                onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}
              >
                ...
                <div className="text-right">
                  <div className="text-base font-bold" style={{ fontFamily: 'Syne, sans-serif', color: info.color }}>
